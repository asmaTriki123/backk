...
// authentification lavage



/*routes.post("/loginS", (req, res) => {
    Station.findOne({email: req.body.email}).exec().then((station) => {
if(!station){
    return res.status(401).json({
        message0: "Station n'est pas trouvée",
        status: false,
        data: undefined
    })
}
bcrypt.compare(req.body.password, station.password, async(err, result) => {
    ...
    if(result){
        ...
        await Token.findOneAndUpdate({_stationId: station._id   , tokenType: "login"},  {new: true, upsert: true})
       return res.status(200).json({
      status: true, 
      message: "login successfuly",
